...
u.setAttribute('id', 'list');
u.classList.add('list');
d.appendChild(u);
document.body.append(d);
let di = document.createElement('div');
di.setAttribute('id', 'a');
document.body.append(di);

/* no.of records per page needs to be only entered here below */

let noOfPages = 5;
let pages;
let pagination = x.length / noOfPages;
pagination - Math.floor(pagination) !== 0 ? pages = Math.floor(pagination) + 1 : pages = pagination;
console.log(pages);
...
for (let i = 1; i <= pages; i++) {
    ...
    a.setAttribute('id', i);
    a.addEventListener('click', _onAnchorClick);
    document.getElementById('a').appendChild(a);
}
document.getElementById('a').insertBefore(prev, document.getElementById("2"));
...
function fetchData(index) {
    let j = 0;
    if (index !== 1) {
        for (let i = 1; i < document.getElementById('list').childNodes.length; i++) {
            document.getElementById('list').childNodes[i].innerText = '';
        }
    }
    for (let i = noOfPages * (index - 1); i <= noOfPages * (index - 1) + noOfPages - 1; i++ , j++) {
        if (x[i] !== undefined) {
            let name = x[i].name;
            let email = x[i].email;
            document.getElementById('list').childNodes[j].innerHTML = name + "    " +" - "+"    "+ email + '<br></br>';
        }
    }
}
function _onAnchorClick(e) {
    let prevEl = document.getElementsByClassName('is-selected')[0];
    ...
    if (e.currentTarget.id === "Prev") {
        if(prevEl.id ==='1'){
            document.getElementById('a').lastChild.classList.add('is-selected');
            fetchData( document.getElementById('a').lastChild.id);
        }else{
        document.getElementById(prevEl.id - 1).classList.add('is-selected');
        fetchData(prevEl.id - 1);
        }
    } else {
...
fetchData(1);
document.getElementById("1").classList.add("is-selected");